...
//读取PUSH_PLUS_TOKEN
function getPushPlusToken() { if ($.isNode()) { if (process.env.PUSH_PLUS_TOKEN) { return process.env.PUSH_PLUS_TOKEN } else { return false } } else { if ($argument?.pushplusToken !== "") { return $argument?.pushplusToken } else { $.log("请先填写变量：PUSH_PLUS_TOKEN");  return false } } }
